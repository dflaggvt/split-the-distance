...
CREATE POLICY "sessions_admin_read" ON sessions FOR SELECT USING (
  (auth.jwt() -> 'app_metadata' ->> 'role') = 'admin'
  OR auth.role() = 'service_role'
);

DROP POLICY IF EXISTS "session_events_insert_all" ON session_events;
CREATE POLICY "session_events_insert_all" ON session_events FOR INSERT WITH CHECK (true);

DROP POLICY IF EXISTS "session_events_admin_read" ON session_events;
CREATE POLICY "session_events_admin_read" ON session_events FOR SELECT USING (
  (auth.jwt() -> 'app_metadata' ->> 'role') = 'admin'
  OR auth.role() = 'service_role'
);
