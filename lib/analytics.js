...
/**
 * Log session start (called once when session is created)
 */
async function logSessionStart() {
  ...
  try {
    ...
    console.log('[STD] Session started:', currentSessionId)
  } catch (err) {
...
/**
 * Log when user shares (copies link or uses native share)
 */
export async function logShare(data) {
  ...
  try {
    await supabase.from('shares').insert({
      session_id: getSessionId(),
      share_type: data.shareType, // 'copy_link' or 'native_share'
      from_name: data.fromName,
      to_name: data.toName,
      share_url: data.shareUrl,
      is_internal: isInternalUser(),
    })
    console.log('[STD] Share logged:', data.shareType)
  } catch (err) {
