 ...
 */
export function calculateTimeMidpoint(leg) {
...
/**
 * Calculate the midpoint along the route based on distance.
 * Walks along the route steps using per-step distance (meters).
 */
export function calculateDistanceMidpoint(leg) {
  ...
  for (let i = 0; i < steps.length; i++) {
    const step = steps[i];
    const stepDistance = step.distance.value; // meters
    const path = step.path; // array of LatLng objects

    if (accumulated + stepDistance >= halfDist) {
      // The midpoint is within this step
      const remaining = halfDist - accumulated;
      const fraction = stepDistance > 0 ? remaining / stepDistance : 0;

      if (path && path.length >= 2) {
        // Walk along the path points within this step
        const result = interpolateAlongPath(path, fraction, 0);
        return { lat: result.lat, lon: result.lon, distanceFromStart: halfDist };
      }
 ...
 */
function getDistanceMatrix(origins, destination, travelMode = 'DRIVING') {
  return new Promise((resolve, reject) => {
 ...
 * Minimizes the maximum drive time or distance (so no one gets screwed).
 * 
 * @param {Array} locations - Array of {lat, lon, name} objects (3-6 locations)
 * @param {Object} options - { mode: 'time' | 'distance' }
 * @returns {Promise<Object>} - { midpoint, driveTimes, maxDrive, minDrive, spread, optimizeBy }
 */
export async function getMultiLocationMidpoint(locations, { mode = 'time', travelMode = 'DRIVING' } = {}) {
  if (locations.length < 2) {
    throw new Error('Need at least 2 locations');
  }

  // Which field to optimize: 'duration' (seconds) or 'distance' (meters)
  const field = mode === 'distance' ? 'distance' : 'duration';
  // Convergence threshold: 10 min for time, ~16 km (~10 mi) for distance
  const threshold = mode === 'distance' ? 16000 : 600;
  ...
  if (locations.length === 2) {
    ...
    return {
      ...
      driveTimes: [
        { origin: locations[0], duration: route.totalDuration / 2, durationText: formatDuration(route.totalDuration / 2), distance: route.totalDistance / 2 },
        { origin: locations[1], duration: route.totalDuration / 2, durationText: formatDuration(route.totalDuration / 2), distance: route.totalDistance / 2 },
      ],
  ...
  for (let i = 0; i < MAX_ITERATIONS; i++) {
    try {
      const driveTimes = await getDistanceMatrix(locations, candidate, travelMode);
      ...
      const maxVal = Math.max(...driveTimes.map((dt) => dt[field]));
      const minVal = Math.min(...driveTimes.map((dt) => dt[field]));
      ...
      // If spread is small enough, we're done
      if (spread < threshold) {
        break;
      }
      
      // Find the "loser" — person with the longest drive/distance
      const loserIdx = driveTimes.findIndex((dt) => dt[field] === maxVal);
  ...
  if (!bestDriveTimes) {
    bestDriveTimes = await getDistanceMatrix(locations, bestCandidate, travelMode);
  }
...
/**
 * Calculate stop points along a route at regular intervals.
 * Walks along leg.steps[] and records a point every N minutes or N miles.
 *
 * @param {Object} leg - Google Directions leg object
 * @param {{ interval: number, mode: 'time' | 'distance' }} options
 *   - mode 'time': interval is in minutes
 *   - mode 'distance': interval is in miles
 * @returns {Array<{ index: number, lat: number, lon: number, timeFromStart: number, distanceFromStart: number }>}
 */
export function calculateStopPoints(leg, { interval, mode = 'time' } = {}) {
  const steps = leg.steps;
  if (!steps || steps.length === 0 || !interval) return [];

  const totalDuration = leg.duration.value; // seconds
  const totalDistance = leg.distance.value; // meters

  // Convert interval to base units
  const intervalBase = mode === 'distance'
    ? interval * 1609.344  // miles → meters
    : interval * 60;       // minutes → seconds

  // Thresholds: skip stop points too close to destination
  const minRemainingTime = 15 * 60;      // 15 minutes in seconds
  const minRemainingDist = 15 * 1609.344; // 15 miles in meters

  const stops = [];
  let accTime = 0;
  let accDist = 0;
  let nextTarget = intervalBase; // First stop target
  let stopIndex = 1;
  ...
  for (let i = 0; i < steps.length; i++) {
    ...
    // Check if next target falls within this step
    while (true) {
      const accVal = mode === 'distance' ? accDist : accTime;
      const stepVal = mode === 'distance' ? stepDistance : stepDuration;

      if (accVal + stepVal < nextTarget) break; // Target is beyond this step

      // Target is within this step — interpolate the point
      const remaining = nextTarget - accVal;
      const fraction = stepVal > 0 ? remaining / stepVal : 0;

      let lat, lon;
      if (path && path.length >= 2) {
        const pt = interpolateAlongPath(path, fraction, 0);
        lat = pt.lat;
        lon = pt.lon;
      } else {
        const sLat = step.start_location.lat();
        const sLng = step.start_location.lng();
        const eLat = step.end_location.lat();
        const eLng = step.end_location.lng();
        lat = sLat + fraction * (eLat - sLat);
        lon = sLng + fraction * (eLng - sLng);
      }

      // Calculate time and distance at this stop
      const stopTime = accTime + fraction * stepDuration;
      const stopDist = accDist + fraction * stepDistance;

      // Skip if too close to destination
      const remainingTime = totalDuration - stopTime;
      const remainingDist = totalDistance - stopDist;
      if (remainingTime < minRemainingTime || remainingDist < minRemainingDist) {
        break; // Don't add this or any further stops
      }
      ...
      stops.push({
        ...
        lat,
        lon,
        timeFromStart: stopTime,
        distanceFromStart: stopDist,
      });

      stopIndex++;
      nextTarget += intervalBase;

      // Safety: don't exceed total route
      if (nextTarget > (mode === 'distance' ? totalDistance : totalDuration)) break;
    }

    accTime += stepDuration;
    accDist += stepDistance;

    // Safety: if we've already passed the total, stop
    if (nextTarget > (mode === 'distance' ? totalDistance : totalDuration)) break;
  }
